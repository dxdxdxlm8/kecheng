...
/**
 * 模型偶尔会把历史消息里的角色标签（[小王] / [学生] / 教师：等）当成自己的输出格式照抄，
 * 导致学生看到"[小王] 哈哈，我也刚理出个头绪"这类脏开头。这里在落库和推流前统一剥掉。
 */
const ROLE_LABEL_RE = /^\s*(?:\[\s*(?:小王|学生|教师|老师|助教|助手|AI|ai)\s*\]|(?:小王|学生|教师|老师|助教|助手)\s*[:：])\s*/;
...
请和小王一起讨论这道题的建模思路，想想怎么把实际问题转化为圆与直线位置关系的数学问题。讨论完后，输入"练习"开始做课堂练习题。`;
...
14. 【重要】回复正文绝对不要以任何角色标签开头（如"[小王]""[学生]""小王："），直接说出你要说的话
...
9. 【重要】回复正文绝对不要以任何角色标签开头（如"[小王]""[学生]""小王："），直接说出你要说的话
...
export async function POST(request: NextRequest) {
  ...
  const readableStream = new ReadableStream({
    async start(controller) {
      let fullResponse = '';
      // 角色标签只可能出现在开头几个字，先攒一小段判定完再推流，避免学生看到"[小王] ..."
      let headBuffer = '';
