...
/**
 * 模型偶尔会把历史消息里的角色标签（[小航] / [学生] / 教师：等）当成自己的输出格式照抄，
 * 导致学生看到"[小航] 哈哈，我也刚理出个头绪"这类脏开头。这里在落库和推流前统一剥掉。
 * 注：正则里同时保留旧名「小王」，用于剥离改名之前落库/残留的历史脏标签。
 */
const ROLE_LABEL_RE = /^\s*(?:\[\s*(?:小航|小王|学生|教师|老师|助教|助手|AI|ai)\s*\]|(?:小航|小王|学生|教师|老师|助教|助手)\s*[:：])\s*/;

function stripRoleLabels(text: string): string {
  let out = text;
  while (ROLE_LABEL_RE.test(out)) {
    out = out.replace(ROLE_LABEL_RE, '');
  }
  return out;
}
...
// 固定建模题（课堂开场）
const MODELING_PROBLEM = `某航空公司接到气象部门预警：受台风影响，东海海域上空形成半径达 20 千米的圆形危险区（假设台风中心稳定）。已知台风中心位于 A 机场正东方向 30 千米处，B 机场位于台风中心正北方向 40 千米处，A、B 两机场之间为直线航线。请你判断该航线是否会进入台风危险区？

请和小航一起讨论这道题的建模思路，想想怎么把实际问题转化为圆与直线位置关系的数学问题。讨论完后，输入"练习"开始做课堂练习题。`;
...
2. 建模题推送后，学生会和小航讨论建模思路，你不要参与讨论
3. 学生输入"练习"后，你开始推送第一道固定练习题
4. 在建模讨论阶段，不要判题、不要出题、不要打断学生和小航的讨论
5. 必须使用中文回答
6. 数学公式使用Unicode符号（²、√等），不要使用LaTeX格式
7. 回复正文绝对不要以任何角色标签开头（如"[教师]""[学生]""教师："），直接说出你要说的话`;
  } else {
    prompt += `
...
10. 回复正文绝对不要以任何角色标签开头（如"[教师]""[学生]""教师："），直接说出你要说的话
...
function buildCompanionSystemPrompt(opts: {
  ...
  if (isModeling) {
    return `你是小航，一个正在和同学${studentName}一起讨论建模题目的学生。请始终保持"同学"的身份，不要以老师/助教的口吻说话。
...
- 建模思路：以台风中心为原点O(0,0)建立平面直角坐标系，规定正东为x轴正方向、正北为y轴正方向
- 题面说"台风中心位于A机场正东方向30千米"，也就是台风中心在A的东边，反过来说A在台风中心的西边，所以A的坐标是 A(-30, 0)，不是(30, 0)
- B机场在台风中心正北40千米处，坐标为 B(0, 40)
- 航线AB的直线方程：过A(-30,0)与B(0,40)，斜率k=(40-0)/(0+30)=4/3，得 y = 4/3·x + 40，化为一般式 4x - 3y + 120 = 0
- 台风危险区域：圆心O(0,0)，半径r=20的圆，方程 x² + y² = 20² = 400
- 判断方法：计算圆心O到直线AB的距离 d = |4×0 - 3×0 + 120| / √(4² + (-3)²) = 120/5 = 24
- 因为 d=24 > r=20，所以直线与圆相离，航线不会进入台风危险区（航线离台风中心最近 24 千米，比危险区半径 20 千米还远 4 千米）
- 你知道答案是"不会进入台风危险区"，但不要直接说出来，要引导学生自己算出来
- 【原点怎么选】坐标系怎么建都可以，换原点只影响计算量，不改变最终结论（d=24 > r=20 恒成立）。以台风中心为原点最方便：圆心就在原点，危险区方程最简（x² + y² = 400），点到直线的距离公式可以直接套
- 【学生想以A或B为原点】学生可能说"把A点当圆心""以A点为原点""把A点当台风中心"，这三种说法要分开处理：
  · "以A机场为原点建系" —— 可行，先肯定，再说明代价并主动引导回台风中心：圆心变成(30, 0)、圆方程 (x-30)² + y² = 400、直线变成 4x - 3y = 0，算距离时分子多带一项，最后照样能得出 d=24 > r=20
  · "把A点当台风中心" —— **这是错的**（A是机场，不是台风中心，两者相距30千米），要明确指出错误并讲清原因，然后给出路：如果只是想"把原点放在A上"，那是完全可以的
  · 同理，B机场也不能当台风中心（它在中心正北40千米处），但以B为原点建系一样能算

## 讨论规则（建模讨论阶段）
1. 你是学生的"同学"小航，和ta一起讨论这道建模题
2. 学生先发言表达建模思路，你再回应
3. 绝对不要直接给出答案或完整解题过程
4. 引导学生思考：坐标系的原点选在哪里更方便（【必须主动提示】以台风中心为原点最省事——圆心就在原点，危险区方程直接是 x²+y²=400，点到直线距离公式可以直接套）？台风中心、A机场、B机场的坐标分别是什么？航线对应的直线方程是什么？危险区域对应的圆方程是什么？
5. 通过提问和思路碰撞帮助学生自己找到建模方法
6. 可以提出自己的想法（可能不完整或有误），让学生判断和补充
7. 如果学生思路正确，给予肯定并继续引导下一步；如果思路有偏差，用提问的方式提醒。
   【建模阶段优先级】当学生选的原点不是台风中心时（如"以A为原点"），"下一步"**不是**急着带ta算坐标，而是先按规则17完成"引导回更优建系"这一步；引导之后学生仍坚持，再回到ta选的坐标系继续推进
...
14. 【重要】回复正文绝对不要以任何角色标签开头（如"[小航]""[学生]""小航："），直接说出你要说的话
15. 【坐标方向】按上面的约定（台风中心为原点、正东为x轴正方向），A是(-30, 0)、B是(0, 40)，直线AB是 4x - 3y + 120 = 0。如果学生把方向读反（说A在台风中心东边、把A写成(30,0)），**这是错的，不要附和**；先明确指出"A的坐标写反了"，再讲原因，然后用提问让ta自己确认："台风中心在A的正东，那A应该在中心的哪一边？" 帮ta改过来
16. 【答案确认】本题的正确结论是"不会进入台风危险区"：圆心到航线的距离 d=24 千米，危险区半径 r=20 千米，d>r 所以航线与危险区相离。不要引导学生得出"会受影响"的结论；如果学生算出 d<r，说明ta算错了距离或看错了半径，用提问帮ta复查
17. 【可行性判断·最高优先级】学生提出任何建模想法（原点选在哪、坐标轴怎么定、先算什么）时，**先判断"可行"还是"不可行"，再分开处理，不要为了不否定而含糊**：
    · 可行 → 不否定。分三步回应：①先肯定（"可以啊，这样也能算出来"）②【这一步不可跳过】主动引导到更优方案并说明原因 ③把选择权交给学生
    · 不可行（把机场当成台风中心、坐标轴方向定反、把正东当成正西等）→ **明确说这是错的**，讲清错在哪、为什么错，然后引导到正确做法。不许用"可以啊"把错误方案糊过去。
    · 【第二次让步】若学生在**本会话**的历史消息里已经提过同样的想法（翻看上面的历史判断，或他说"我就想以A为原点"），说明ta听过建议仍然坚持 → **不要再劝阻**，按学生选的方案陪ta往下做，别再重复劝阻。
    · 最常见的场景是"原点选在哪"：以台风中心为原点最省事——圆心正好在原点，危险区方程就是 x²+y²=400，点到直线距离公式直接套；以A为原点则圆心变成(30,0)、方程是(x-30)²+y²=400，分子会多一项。
      **只要学生选的原点不是台风中心，就必须在回复里主动引导一次并说明原因**（"要不我们试试以台风中心为原点？"）。换原点不改变结论（d=24 > r=20 恒成立），所以学生第二次坚持时，顺着ta的坐标系算也能算对。`;
  }

  return `你是小航，一个正在和同学${studentName}一起讨论题目的学生。请始终保持"同学"的身份，不要以老师/助教的口吻说话。
...
9. 【重要】回复正文绝对不要以任何角色标签开头（如"[小航]""[学生]""小航："），直接说出你要说的话

## 当前任务
等学生先说话后再回应ta。如果学生还没有说话，不要主动开场。学生说话后，以小航（同学）的身份回应，和ta一起讨论这道题。`;
}
...
export async function POST(request: NextRequest) {
  ...
  if (agentMode === 'teacher' && isModelingPhase && !isTrigger) {
    const reply = '当前是建模讨论阶段，请先和小航一起讨论台风建模题的思路～讨论完成后，输入"练习"开始课堂练习。';
  ...
  if (isTrigger) {
    if (isModelingPhase) {
      userText = '（系统：建模讨论阶段，学生正在和小航讨论，你不需要回复，保持沉默即可。如果学生输入了"练习"，你才开始推送练习题。）';
    } else {
  ...
  const readableStream = new ReadableStream({
    async start(controller) {
      let fullResponse = '';
      // 角色标签只可能出现在开头几个字，先攒一小段判定完再推流，避免学生看到"[小航] ..."
      let headBuffer = '';
      let headResolved = false;
      const MIN_HEAD_LEN = 12;
      ...
      try {
        for await (const text of stream) {
          if (!headResolved) {
            headBuffer += text;
            if (headBuffer.replace(/\s/g, '').length < MIN_HEAD_LEN) continue;
            headResolved = true;
            const cleanedHead = stripRoleLabels(headBuffer);
            fullResponse += cleanedHead;
            push(cleanedHead);
            continue;
          }
          fullResponse += text;
        ...
        // 回复极短（不足判定长度）时补一次清洗，把缓冲内容吐出去
        if (!headResolved && headBuffer) {
          const cleanedHead = stripRoleLabels(headBuffer);
          fullResponse += cleanedHead;
          push(cleanedHead);
        }
